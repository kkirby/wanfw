...
- `configSchema` (optional): a path to a JSON Schema file validating this plugin's own config block in the framework/service document, if it has settings beyond capabilities. Populate `title`/`description` on every field -- this text renders directly as the field's label and help text in tier1's service editor (`SchemaForm`, `packages/tier1/components/schema-form/`), it's not just documentation. A bare property name with no description is exactly the kind of unlabeled, unexplained field this project is trying to get away from -- see `plugins/deploy-docker/config-schema.json` for the reference example.
