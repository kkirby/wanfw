    ...
    run: async (ctx: ReconcileRunContext): Promise<StageResult> => {
      ...
      for (const [serviceId, doc] of desiredState.services) {
        ...
        const statusDoc: ServiceStatusDoc = {
          serviceId,
          phase: computeServicePhase(serviceId, gate, container?.state === "running", undefined),
          endpoints: expose?.hostname ? [expose.hostname] : [],
          certNotAfter: null, // real ACME lands in T4.x; internal-CA mode has no expiry to report yet
